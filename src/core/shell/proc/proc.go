...
func parseCommand(cmd string) (args []string) {
	...
	array := string2.SplitAndTrim(cmd, " ")
	for _, v := range array {
		if len(argStr) > 0 {
			argStr += " "
		}
...
func searchBinaryPath(path string) string {
	...
	case "windows":
		...
		if strings.Contains(envPath, ";") {
			array = string2.SplitAndTrim(envPath, ";")
	...
	default:
		array = string2.SplitAndTrim(env.Get("PATH"), ":")
	}
	if len(array) > 0 {
		fullpath := ""
		for _, v := range array {
			fullpath = v + file.Separator + path
